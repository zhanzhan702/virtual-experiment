...
argument-hint: "使用 LeaferJS 绘制/创建/操作 Canvas 图形"
user-invocable: true
---
...
## Project Context (虚拟实验平台)

本项目已在以下位置使用 Leafer UI：

- 依赖：`leafer-ui` + `@leafer-ui/partner`
- `frontend/src/components/HighVoltage/HMeteringRoomCanvas.vue`：计量小室操作画布（步骤5 挂电表 / 步骤6 接线盒开关），背景层 + 交互层（热区 Rect），画布尺寸与背景图渲染尺寸一致防缩放错位
- `frontend/src/views/HighVoltage/HCabinetLocalView.vue`：步骤3/4 保持 HTML 实现，步骤5+ 渲染计量小室画布
- 鼠标跟随用 Vue HTML 层实现（不进画布），复用 cursor-following 模式

典型用法模式（HMeteringRoomCanvas）：

```js
import { Leafer, Group, Image, Rect, Event } from 'leafer-ui'

const leafer = new Leafer({ view: el, width, height })
const bgLayer = new Group()
const hitLayer = new Group()
leafer.add(bgLayer)
leafer.add(hitLayer)
bgLayer.add(new Image({ url, x: 0, y: 0, width, height }))
const zone = new Rect({ x, y, width, height, fill: 'rgba(0,0,0,0)' })
zone.on(Event.CLICK, () => {})
hitLayer.add(zone)
// 画布级事件：leafer.on(Event.CLICK, e => e.getLocal()) 取画布局部坐标
```

画布相关图片资源位于 `frontend/src/assets/images/cabinet/metering-room/`，通过 `constants/images.js` 统一引用（`Images.meteringRoomNoMeter` 等）。
